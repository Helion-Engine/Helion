...
public partial class Inventory
{
    private sealed class WeaponData(string name, string ammo, int amount, int use)
    {
        ...
        public int AmmoStartAmount { get; set; } = amount;
        public int AmmoUseAmount { get; set; } = use;
    }
    ...
    [Fact(DisplayName = "Remove all weapons and switch")]
    public void RemoveAllWeapons()
    {
        ...
        var slot = Player.Inventory.Weapons.GetNextSlot(Player, 3);
        slot.Slot.Should().Be(-1);
        slot.SubSlot.Should().Be(-1);
