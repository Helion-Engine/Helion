...
public class Scroll
{
    ...
    public void Action2085_ScrollSidesToSectorMovement()
    {
        ...
        var offset = line.Front.ScrollData!.GetOffset(WallLocation.Middle, ScrollOffsetType.Current);
        GameActions.TickWorld(World, 1);
    ...
    public void Action2086_ScrollSidesAccelerateToSectorMovement()
    {
        ...
        line.Front.ScrollData.GetOffset(WallLocation.Middle, ScrollOffsetType.Current).Should().NotBe(offset);

        GameActions.ActivateLine(World, Player, 37, ActivationContext.UseLine);
        GameActions.RunSectorPlaneSpecial(World, GameActions.GetSectorByTag(World, 5), () => { });
        offset = line.Front.ScrollData!.GetOffset(WallLocation.Middle, ScrollOffsetType.Current);
        GameActions.TickWorld(World, 1);
        // Scroll stops moving with sector back to start height
        line.Front.ScrollData.GetOffset(WallLocation.Middle, ScrollOffsetType.Current).Should().Be(offset);
    }
    ...
    { 
        ...
        line.Front.ScrollData!.GetOffset(WallLocation.Middle, ScrollOffsetType.Current).Should().Be(offset);
        line.Back.ScrollData!.GetOffset(WallLocation.Middle, ScrollOffsetType.Current).Should().Be(new Vec2D(-offset.X, offset.Y));
    }
}