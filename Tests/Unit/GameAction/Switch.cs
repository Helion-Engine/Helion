...
public class Switch
{
    ...
    [Fact(DisplayName = "Multi use switch starts on")]
    ...
    [Fact(DisplayName = "Multi use switch starts off")]
    public void MultiUseSwitchOff()
    {
        const int Line = 23;
        var line = GameActions.GetLine(World, Line);
        var side = line.Front;
    ...
    {
        // Doom had a quirk where if the switch could be activated during the animation it would switch the texture back off
        const int Line = 34;
        var line = GameActions.GetLine(World, Line);
        var side = line.Front;
