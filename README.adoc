...
Helion uses https://dotnet.microsoft.com/en-us/download/dotnet/9.0[.NET 9]. The self-contained Helion releases include it, otherwise Windows users will need to install the Desktop Runtime, and Linux users will need to follow the instructions https://learn.microsoft.com/en-us/dotnet/core/install/linux[for their distro]. On Linux, you will also need to make sure OpenAL, libsndfile, and libmpg123 are installed.
