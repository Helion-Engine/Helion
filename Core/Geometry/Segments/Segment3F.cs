...
{
    public class Segment3F<T> where T : Vector3F
    {
        public T Start;
        public T End;
        ...
        public Segment3F(T start, T end)
        {
            Start = start;
            End = end;
        }
        ...
        public static Seg3F operator +(Segment3F<T> self, Vec3F other) => new(self.Start + other, self.End + other);
        public static Seg3F operator +(Segment3F<T> self, T other) => new(self.Start + other, self.End + other);
        public static Seg3F operator -(Segment3F<T> self, Vec3F other) => new(self.Start - other, self.End - other);
        public static Seg3F operator -(Segment3F<T> self, T other) => new(self.Start - other, self.End - other);
        public static bool operator ==(Segment3F<T> self, Segment3F<T> other) => self.Start == other.Start && self.End == other.End;
        public static bool operator !=(Segment3F<T> self, Segment3F<T> other) => !(self == other);

        public T Opposite(Endpoint endpoint) => endpoint == Endpoint.Start ? End : Start;
        public Vec3F FromTime(float t) => Start + (Delta * t);
        public override string ToString() => $"({Start}), ({End})";
        public override bool Equals(object? obj) => obj is Segment3F<T> seg && Start == seg.Start && End == seg.End;
