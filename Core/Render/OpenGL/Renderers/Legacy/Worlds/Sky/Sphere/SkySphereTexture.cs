...
namespace Helion.Render.OpenGL.Renderers.Legacy.Worlds.Sky.Sphere
{
    public class SkySphereTexture : IDisposable
    {
        ...
        public SkySphereTexture(Resources resources, IGLFunctions functions, LegacyGLTextureManager textureManager)
        {
        ...
        {
            return m_resources.Textures.GetOnly(DefaultSky, Namespace.Textures).Image;
        }
        ...
        private GLLegacyTexture CreateTexture(Bitmap fadedSkyImage, string debugName = "")
        {
            ...
            Image image = new(fadedSkyImage);

            // TODO: This is not portable to a modern renderer because it uses a fake index.
            Texture texture = new("FADED_SKY", image, Namespace.Flats, 0);

            GLLegacyTexture glTexture = new(textureId, debugName, dimension, gl, TextureTargetType.Texture2D, texture);

            m_textureManager.UploadAndSetParameters(glTexture, image, debugName, Namespace.Global);
