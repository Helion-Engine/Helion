...
namespace Helion.Render.OpenGL
{
    public class GLRenderer : IRenderer, IDisposable
    {
        ...
        public static mat4 CalculateMvpMatrix(RenderInfo renderInfo)
        {
            ...
            float zNear = (float)((renderInfo.ViewerEntity.LowestCeilingZ - renderInfo.ViewerEntity.HighestFloorZ - renderInfo.ViewerEntity.ViewHeight) * 0.75);
            zNear = MathHelper.Clamp(zNear, 0.5f, 7.9f);
            mat4 projection = mat4.PerspectiveFov(fovY, w, h, zNear, 8192.0f);
        ...
        private void HandleDrawImage(DrawImageCommand cmd)
        {
            if (cmd.AreaIsTextureDimension)
            {
                ...
                m_hudRenderer.DrawImage(cmd.TextureName, cmd.DrawArea, cmd.MultiplyColor, cmd.Alpha);
        }
