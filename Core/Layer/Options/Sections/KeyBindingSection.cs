...
public class KeyBindingSection : IOptionSection
{
    ...
    public void Render(IRenderableSurfaceContext ctx, IHudRenderContext hud, int startY)
    {
        ...
        {
            hud.Text(command, Fonts.SmallGray, fontSize, (-xOffset, y), out Dimension commandArea, window: Align.TopMiddle, 
                anchor: Align.TopRight, color: Color.Red);
            hud.Text(key.ToString(), Fonts.SmallGray, fontSize, (xOffset, y), out Dimension keyArea, window: Align.TopMiddle, 
                anchor: Align.TopLeft, color: Color.White);
