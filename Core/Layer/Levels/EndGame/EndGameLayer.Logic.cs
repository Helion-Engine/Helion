...
public partial class EndGameLayer
{
    ...
    private void SetNextCastEntity()
    {
        m_castFrameCount = 0;
        m_castIndex++;
        m_castIndex %= CastData.Cast.Count;
        m_castEntity = World.EntityManager.Create(CastData.Cast[m_castIndex].DefinitionName, Vec3D.Zero);        
    ...
    private string GetCastEntitySound()
    {
        ...
        if (name.EqualsIgnoreCase("DoomImp") && frameDiff == 2)
            return "imp/melee";
        if (name.EqualsIgnoreCase("Demon") && frameDiff == 1)
            return "demon/melee";
        if (name.EqualsIgnoreCase("LostSoul") && frameDiff == 1)
            return "skull/melee";
        if (name.EqualsIgnoreCase("Cacodemon") && frameDiff == 1)
            return "caco/attack";
        if ((name.EqualsIgnoreCase("HellKnight") || name.EqualsIgnoreCase("BaronOfHell")) && frameDiff == 1)
            return "baron/attack";
        if (name.EqualsIgnoreCase("Arachnotron") && frameDiff == 1)
            return "baby/attack";
        if (name.EqualsIgnoreCase("PainElemental") && frameDiff == 1)
            return "skull/melee";
        if (name.EqualsIgnoreCase("FatSo") && (frameDiff == 1 || frameDiff == 4 || frameDiff == 7))
            return "fatso/attack";
        if (name.EqualsIgnoreCase("Archvile") && frameDiff == 1)
            return "vile/firecrkl";
        if (name.EqualsIgnoreCase("SpiderMastermind") && (frameDiff == 1 || frameDiff == 2))
            return "spider/attack";
        if (name.EqualsIgnoreCase("Cyberdemon") && (frameDiff == 1 || frameDiff == 3 || frameDiff == 5))
            return "weapons/rocklf";
        if (name.EqualsIgnoreCase("DoomPlayer") && frameDiff == 0)
