...
public partial class WorldLayer
{
        ...
        bool automapVisible, StatusBarLayoutDef? activeLayout)
    {
        ...
        else if (m_statusBarSizeType == StatusBarSizeType.MinimalWithAmmo) { DrawMinimalStatusBarWithAmmo(hud, topRightY); return; }
    ...
    private void DrawMinimalStatusBarWithAmmo(IHudRenderContext hud, int topRightY)
    {
        DrawMinimalHudHealthAndArmor(hud);
        DrawMinimalHudKeys(hud, topRightY);
        DrawMinimalHudAmmo(hud);
        DrawMinimalTotalAmmo(hud);
    }
    ...
    {	
        ...
        DrawMinimalTotalAmmoText(hud, "RocketAmmo", m_padding - (int)m_rckt);
        DrawMinimalTotalAmmoText(hud, "Cell", m_padding - (int)m_cell);
    }

    void DrawMinimalTotalAmmoText(IHudRenderContext hud, string ammoName, int y)
    {
        ...
        hud.Text(m_ammoString.AsSpan(), YellowFontName, m_fontHeight, (x, y), both: Align.BottomRight, alpha: m_hudAlpha);
    }
