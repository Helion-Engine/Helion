...
public class GameLayerManager
{
    ...
    public bool HasAnyLayers<TLayer1, TLayer2>() 
        where TLayer1 : GameLayer 
        where TLayer2 : GameLayer
    {
        return TryGet<TLayer1>(out _) || TryGet<TLayer2>(out _);
    }

    public bool AnyLayerWithPriorityOverThis(GameLayer layer)
    {
        for (int i = 0; i < m_layers.Count; i++)
            if (m_layers[i].Priority < layer.Priority)
                return true;
    ...
    public void Render(IRenderableSurfaceContext surface, IHudRenderContext ctx)
    {
        for (int i = m_layers.Count - 1; i >= 0; i--)
            m_layers[i].Render(surface, ctx);
    }
}