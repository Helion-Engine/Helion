...
namespace Helion.Worlds.Geometry.Sectors
{
    public class Sector
    {
        ...
        /// </summary>
        public readonly List<Sector3DFloor> Floors3D = new();
        ...
        public double GetShortestLower()
        {
            ...
            for (int i = 0; i < Lines.Count; i++)
            {
                ...
                if (line.Front.Lower != null)
                {
                    Texture texture = line.Front.Lower.Texture.Texture;
                    if (texture.Image.Height < min)
                        min = texture.Image.Height;
                }

                if (line.Back?.Lower != null)
                {
                    Texture texture = line.Back.Lower.Texture.Texture;
                    if (texture.Image.Height < min)
                        min = texture.Image.Height;
                }
            }
